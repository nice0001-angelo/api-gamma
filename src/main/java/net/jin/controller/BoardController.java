...
public class BoardController {
	...
	public ResponseEntity<List<Board>> list() throws Exception {
		System.out.println("1st test for committing git");
	...
	public ResponseEntity<Board> insert(@Validated @RequestBody Board board,
	...
	// 삭제
	// @PreAuthorize("hasRole('MEMBER' and principal.username == #writer) or
	// hasRole('ADMIN')")
	@RequestMapping(value = "/{boardNo}", method = RequestMethod.DELETE)
	public ResponseEntity<Void> delete(@PathVariable Long boardNo, @RequestParam("writer") String writer)
			throws Exception {
	...
	// 수정
	// @PreAuthorize("hasRole('MEMBER' and principal.username == #board.writer) or
	// hasRole('ADMIN')")
	@RequestMapping(value = "/{boardNo}", method = RequestMethod.PUT)
	public ResponseEntity<Board> update(@PathVariable("boardNo") Long boardNo, @Validated @RequestBody Board board)
			throws Exception {
